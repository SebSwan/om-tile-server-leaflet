...
export class RotatedLatLonProjection {
    θ;
    ϕ;
    ...
    forward(latitude, longitude) {
        ...
        const x2 = Math.cos(this.θ) * Math.cos(this.ϕ) * x +
            Math.cos(this.θ) * Math.sin(this.ϕ) * y +
            Math.sin(this.θ) * z;
        const y2 = -Math.sin(this.ϕ) * x + Math.cos(this.ϕ) * y;
        const z2 = -Math.sin(this.θ) * Math.cos(this.ϕ) * x -
    ...
    reverse(x, y) {
        ...
        const lat2 = -1 *
            Math.asin(Math.cos(this.θ) * Math.sin(lat) - Math.cos(lon) * Math.sin(this.θ) * Math.cos(lat));
        const lon2 = -1 *
            (Math.atan2(Math.sin(lon), Math.tan(lat) * Math.sin(this.θ) + Math.cos(lon) * Math.cos(this.θ)) -
                this.ϕ);
        return [radiansToDegrees(lat2), ((radiansToDegrees(lon2) + 180) % 360) - 180];
    }
}
export class LambertConformalConicProjection {
    ρ0;
    F;
    n;
    λ0;
...
export class LambertAzimuthalEqualAreaProjection {
    λ0;
    ϕ1;
...
export class StereograpicProjection {
    λ0; // Central longitude
    sinϕ1; // Sinus of central latitude
    cosϕ1; // Cosine of central latitude
...
export class ProjectionGrid {
    ...
    findPointInterpolated2D(lat, lon) {
        const [xpos, ypos] = this.projection.forward(lat, lon);
        const x = Math.max(Math.min((xpos - this.origin[0]) / this.dx, this.ranges[1]['end'] - this.ranges[1]['start']), 0);
        const y = Math.max(Math.min((ypos - this.origin[1]) / this.dy, this.ranges[0]['end'] - this.ranges[0]['start']), 0);
        return [x, y];
    }
}
