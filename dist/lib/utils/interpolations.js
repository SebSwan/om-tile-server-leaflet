export const noInterpolation = (values, nx, index) => {
    return Number(values[index]);
};
export const interpolateLinear = (values, nx, index, xFraction, yFraction) => {
...
export const interpolateCardinal2D = (values, nx, index, xFraction, yFraction, tension = 0 // range [0,1]. tension = 0 is Catmull-Rom
) => {
    // Interpolate 4 rows in X
    const r0 = cardinalSpline(xFraction, Number(values[index + -1 * nx - 1]), Number(values[index + -1 * nx + 0]), Number(values[index + -1 * nx + 1]), Number(values[index + -1 * nx + 2]), tension);
    const r1 = cardinalSpline(xFraction, Number(values[index + +0 * nx - 1]), Number(values[index + +0 * nx + 0]), Number(values[index + +0 * nx + 1]), Number(values[index + +0 * nx + 2]), tension);
    const r2 = cardinalSpline(xFraction, Number(values[index + +1 * nx - 1]), Number(values[index + +1 * nx + 0]), Number(values[index + +1 * nx + 1]), Number(values[index + +1 * nx + 2]), tension);
    const r3 = cardinalSpline(xFraction, Number(values[index + +2 * nx - 1]), Number(values[index + +2 * nx + 0]), Number(values[index + +2 * nx + 1]), Number(values[index + +2 * nx + 2]), tension);
    // Interpolate in Y
    return cardinalSpline(yFraction, r0, r1, r2, r3, tension);
};
export const interpolate2DHermite = (values, nx, index, xFraction, yFraction) => {
